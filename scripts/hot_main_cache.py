...
CANONICAL_BUILD_RECIPE = BuildRecipe(
    ...
    version=1,
    dependency_command=("lake", "exe", "cache", "get"),
...
def discover_inputs(project_dir: Path) -> list[Path]:
    """Resolve the three versioned inputs in the local detached clone."""
...
def hash_inputs(project_dir: Path) -> dict[str, str]:
    return {path.name: sha256_file(path) for path in discover_inputs(project_dir)}
...
def hash_inputs_at_commit(repo_root: Path, project_dir: Path, commit: str) -> dict[str, str]:
    try:
...
def git_tracked_changes(repo_root: Path) -> list[str]:
    command = [
        "git",
        "-C",
        str(repo_root),
...
class CacheIdentity:
    ...
    def create(
        ...
        inputs = hash_inputs_at_commit(repo_root, project_dir, commit)
        recipe_payload = recipe.identity_payload()
        payload = json.dumps(
            {"main_commit": commit.lower(), "inputs": inputs, "recipe": recipe_payload},
            sort_keys=True,
            separators=(",", ":"),
        ).encode("ascii")
        return cls(hashlib.sha256(payload).hexdigest(), commit.lower(), inputs, recipe_payload)
...
class HotMainCache:
    ...
    def warm(
        ...
        *,
        ...
        if _test_command_callback is not None and not self.recipe.test_only:
            raise CacheError("a command callback is allowed only with an identity-isolated test recipe")
        ...
        with ExclusiveLock(self.lock_path) as cache_lock:
            ...
            try:
                ...
                if hash_inputs(detached_project) != self.identity.inputs:
                ...
                tracked_changes = git_tracked_changes(checkout)
                if tracked_changes:
                    preview = ", ".join(tracked_changes[:5])
                    raise CacheError(f"tracked source changed during the cache build: {preview}")
    ...
    def _eligible_seed_target(self, supplied_target: Path) -> tuple[Path, Path]:
        ...
        target_inputs = discover_inputs(target_project)
        ...
        if hash_inputs(target_project) != self.identity.inputs:
