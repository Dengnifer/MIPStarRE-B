...
|---|---|
| Paper assumptions | A finite nonempty type graph; one seven-input typed CL sampler machine interface with four variable-arity call modes (dimension, marginal, linear, and factor), bounded raw type codes, and zero output for invalid type codes; one pointwise-total raw-domain seven-input typed decider; fixed-index typed and detyped games whose word widths come from their respective machine runtimes; canonical machine descriptions; finite-game/PCC/value/Schmidt-rank semantics. |
| Lean assumptions | F07 mathematical typed interfaces and F04A game semantics; the existing transitive F06A `ExecutableCLSampler`, returned exactly by `TypedSampler.detype` rather than duplicated in F07A; an F07A-local interpreted finite-program boundary; bounded raw type codes; intrinsically uniformly index-bounded total raw-domain deciders whose times are derived from actual executions over all raw inputs rather than finite game-valid queries, with an attained execution whenever a literal maximum is exposed; explicit fixed-word question/answer transport; fixed-prefix graph/content/padding codecs distinct from self-delimiting variable-tuple/program codecs, each with its own roundtrip and malformed-input laws; no caller-supplied compiler, runtime, transport, bridge, witness, package, producer, or obligation. |
| Paper conclusion | Exact graph event and conditioning law; detyped sampler, decider, and verifier; PCC completeness; value and entanglement soundness with factor `16^|Type|`; level `ell+2`; dimension `4|Type|+s(n)`; separate polynomial runtime bounds: there exists an outermost universal witness `C_s > 0`, before every type set, graph, verifier, and index binder, such that for every `n in N`, `TIME_detype(S)(n) <= C_s * (|TypeId| * TIME_S(n)) ^ C_s` for all positive displayed arguments `|TypeId|` and `TIME_S(n)`; independently, there exists an outermost universal witness `C_d > 0`, before every type set, graph, verifier, and index binder, such that for every `n in N`, `TIME_detype(D)(n) <= C_d * (|TypeId| * TIME_D(n)) ^ C_d` for all positive displayed arguments `|TypeId|` and `TIME_D(n)`; neither runtime clause assumes or derives `0 < n`, and the Lean zero-index convention remains explicitly unresolved; polynomial-time computation of the actual target descriptions. |
| Lean conclusion | The same mathematical constructions and theorem conclusions, including the two separately quantified execution-derived raw-input runtime bounds and interpreted compiled-description bounds; explicit prefix/zero-extension transport for source content and answers; `TypedSampler.detype` returns exactly the existing transitive F06A `ExecutableCLSampler`; the fixed-prefix and self-delimiting/program codecs remain separate; no public repair assumption. |
