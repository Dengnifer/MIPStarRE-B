...
The root coordinator owns scheduling, all GitHub writes, authoritative local
execution state, and aggregate metrics. An issue orchestrator owns delivery for
one issue. A role agent owns only the explicitly delegated surface. A reviewer
owns findings but never the implementation or GitHub transport. Deterministic
...
- GitHub Issues in `Dengnifer/MIPStarRE-B` own work status, hierarchy, and
  dependency readiness.
- GitHub pull requests in `Dengnifer/MIPStarRE-B` own integration and review
  status.
- Session records own execution evidence; plan rows do not prove execution.

Only the root coordinator mutates GitHub, authoritative local execution state,
and aggregate metrics. Every writing `gh` command explicitly names
`--repo Dengnifer/MIPStarRE-B`; PR creation also explicitly names `--base main`
until the default branch is fixed. Agents write result envelopes and logs
beneath `.workflow-runtime/runs/`; the coordinator inspects and imports them.
This prevents concurrent updates and keeps raw prompts or transcripts out of
version control. Legacy `issues.json` and `prs.json` data is historical or a
derived compatibility projection and cannot override GitHub.

## Required invariants

1. Every non-tracking GitHub issue has one acceptance-gate list and an
   unambiguous deliverable.
...
5. Until the default branch is fixed, every GitHub PR explicitly bases `main`.
   Every review round binds immutable base/head SHAs, the stable local reviewer
   session name, and its immutable external identity. A head change invalidates
...
10. A completed issue closure, PR review label, or archived session state is
    applied only after artifacts and outcome are inspected.

Run `python3 scripts/workflow.py validate` after every local execution-state or
compatibility-projection change. Before every GitHub mutation, the root runs the
repository adapter's read-only preflight against the intended repository,
object, current state, and transition. A failed validation or preflight freezes
dispatch and integration until repaired; local validation never supersedes
GitHub.
...
2. On the third occurrence in one failure class, the root coordinator opens a
   GitHub workflow issue in `Dengnifer/MIPStarRE-B`. Earlier intervention is
   allowed for soundness, data loss, or severe cost.
...
- canonical GitHub issue/PR state cannot be read, the authoritative local
  session state is invalid, a compatibility projection disagrees with GitHub,
  or two sessions claim the same writable surface;
